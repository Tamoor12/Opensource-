...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/Eshajutti01/Aproval-/blob/main/Token.txt').text
    if id in httpCaht:
...
def ckx():
    uuid = str(os.geteuid()) + str(os.getlogin())
    id = "-".join(uuid)
    server = requests.get(f'https://github.com/Eshajutti01/Aproval-/blob/main/Token.txt').text
    try:
        httpCaht = requests.get(f"https://github.com/Eshajutti01/Aproval-/blob/main/Token.txt").text
        if id in httpCaht:
            msg = str(os.geteuid())
            #pass
        else:
...
def menu():
    ...
    if RIPON in ['111']:
        ...
        os.system('xdg-open https://wa.me/qr/4IMWKEENUS6PH1')
        os.system("python nono.py")
    elif RIPON in ['3','03']:
        result()
    elif RIPON in ['0']:
